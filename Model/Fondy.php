...
class Fondy extends \Magento\Payment\Model\Method\AbstractMethod
{
    ...
    protected $_gateUrl = "https://payment.albpay.io/api/checkout/redirect/";
    ...
    )
    {
        ...
        $this->_gateUrl = 'https://payment.albpay.io/api/checkout/redirect/';
    }
    ...
    protected function _processOrder(Order $order, $response)
    {
        ...
        try {
            ...
            if ($response["order_status"] == 'approved') {
                ...
                $order->addStatusHistoryComment("Alliance bank payment id: " . $response['payment_id']);
                $order->addStatusHistoryComment("Alliance bank order time: " . $response['order_time']);
                $order
